...
int main(int argC, char* argV[]) {
    ...
    switch (argC) {
     ...
     break;
     default:
     fprintf(stdout, "Usage: %s [Device-ID] [Device-Type] []Obs-Type].\n", argV[0]);
    ...
    if (configFile==NULL)
    {
     fprintf(stdout,"No configuration file found: <%s>.\n", CONFIG_FILE);
     exit(0);
    }
...
//				printf("-> Config file: No Debug.\n");
    ...
    if (strcmp (platformType.c_str() , PROTOCOL_NGSI) == 0)
    {
     smlPath = "NGSI/";
     fileSuffix = ".ngsi";
     resourceDir = "/ngsi10/updateContext";
    }
    ...
    fileOrig = smlPath + "Observation_" + deviceType + "_" + obsType + "_" + deviceID + ".obs";
    fileRepl = smlPath + "Observation_" + deviceType + "_" + obsType + "_" + deviceID + fileSuffix;
    str2find = "DEVICE_TYPE:RPI_ID:DEVICE_ID";
    str2repl = deviceType + ":" + rpiID + ":" + deviceID;

    if( access( fileOrig.c_str(), F_OK ) != -1 ) {
     if (debugSwitch >= 3) fprintf(stdout, "-> Observations file to be sent: <%s>.\n", fileOrig.c_str()); 
    } else {
     fprintf(stdout, "-> No Observations file: <%s>. Add Observations before sending to server.\n", fileOrig.c_str());
     exit(0);
    }

    ret = FindReplaceFile(str2find, str2repl, fileOrig, fileRepl);
	
    if (debugSwitch >= 3) printf("-> RaspberryPI IoT gateway ID (rpiID): <%s>.\n", rpiID.c_str());

    if (debugSwitch >= 3) fprintf(stdout, "-> Observation of a sensor type: <%s>.\n", deviceType.c_str());

    smlFilename = smlPath + "Observation_" + deviceType + "_" + obsType + "_" + deviceID + fileSuffix;
    ...
    if (debugSwitch >= 3) fprintf(stdout, "-> Deleting Observations file: <%s>.\n", fileOrig.c_str());
    ret = remove (fileOrig.c_str());        
}
